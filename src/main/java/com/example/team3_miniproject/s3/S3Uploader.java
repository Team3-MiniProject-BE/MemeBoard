...
public class S3Uploader {
    ...
    public String upload(Long id, MultipartFile multipartFile, String dirName) throws IOException {
        File uploadFile = convert(multipartFile)                                                        // 파일 변환
                .orElseThrow(() -> new IllegalArgumentException("MultipartFile -> File 전환 실패"));
        return upload(id, uploadFile, dirName);                                                         // id, uploadFile(파일이름), 디렉토리 이름 Return
    }
